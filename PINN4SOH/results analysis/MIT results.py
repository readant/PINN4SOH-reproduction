...
import numpy as np
import os
from utils.util import eval_metrix_with_r2  # [修正] MIT results.py 需要 R2（5值）
...
class Results:
    ...
    def parser_label(self):
        ...
        [MAE, MAPE, MSE, RMSE, R2] = eval_metrix_with_r2(pred_label, true_label)
        ...
        for i in range(len(local_minima)):
            ...
            [MAE_i, MAPE_i, MSE_i, RMSE_i, R2_i] = eval_metrix_with_r2(pred_i, true_i)
...
if __name__ == '__main__':
    # [路径统一] 原目录 'revise_results/' 改为 'results/'，与主训练脚本路径保持一致
    root = '../results/MIT results/'
    writer = pd.ExcelWriter('../results/Ours-MIT_results.xlsx')
