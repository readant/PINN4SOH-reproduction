...
def get_args():
    ...
    parser.add_argument('--save_folder',type=str,default='./results/')  # [路径统一] 原为 './results of reviewer/'
...
if __name__ == '__main__':
    ...
    for i in range(len(xjtu_batch_names)):
        ...
        for e in range(10):
            setattr(args,'save_folder',os.path.join('./results/',f'{args.dataset}-{args.model} results/{i}-{i}/Experiment{e+1}'))  # [路径统一] 原为 './results of reviewer/'
            if not os.path.exists(args.save_folder):
                os.makedirs(args.save_folder)
